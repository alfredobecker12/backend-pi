...
class BrandRegisterService {
    async execute({ cnpj_rep, cnpj_marca, razao_social }: UserRequest) {
        try {
            ...
            const representanteExistente = await prismaClient.representanteMarca.findFirst({
                where: {
                    cnpjMarca: cnpj_marca,
                    cnpjRepresentante: cnpj_rep,
                },
            ...
            if (representanteExistente) {
                throw new AppError("Este representante já está associado a esta marca.", 400);
            }
            ...
            if (categoriasAssociadas.has(CatCliente.P) && categoriasAssociadas.has(CatCliente.M) && categoriasAssociadas.has(CatCliente.G)) {
                throw new AppError("Já existem representantes associados à marca nas três categorias.", 400);
            }
